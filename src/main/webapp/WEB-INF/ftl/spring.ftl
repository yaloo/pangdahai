    ...
    <#if options?is_hash>
        <#list options?keys as value>
            <#assign id="${status.expression}${value_index}">
                <input type="radio" id="${id}" name="${status.expression}" value="${value?html}"<#if stringStatusValue == value> checked="checked"<#elseif value_index==1>checked="checked"</#if> ${attributes}<@closeTag/>
 ...
 *    or CSS styles or size
-->
<#macro formCheckboxes path options checkedList separator attributes="">
    <@bind path/>
    <#list options?keys as value>
    <#assign id="${status.expression}${value_index}">
    <#assign isSelected = contains(checkedList, value)>
